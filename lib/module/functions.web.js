...
const WEB_ONLY_SYNC_ERROR = "[op-sqlite] Web backend is async-only. Use openAsync() and async methods like execute().";
function throwSyncApiError(method) {
...
function ensureSingleStatement(sql) {
  ...
  if (sql.includes(";")) {
    ...
    if (!trimmed.endsWith(";") || trimmed.slice(0, -1).includes(";")) {
      console.warn("[op-sqlite] Web execute() runs full SQL strings. Avoid multi-statement SQL for parity with native first-statement behavior.");
    }
  }
}
let promiserPromise = null;
async function getPromiser() {
  if (!promiserPromise) {
    promiserPromise = (async () => {
      ...
      try {
        ...
        throw new Error(`[op-sqlite] Web support requires optional dependency @sqlite.org/sqlite-wasm. Install it in your app: npm i @sqlite.org/sqlite-wasm (or yarn add @sqlite.org/sqlite-wasm). Original error: ${error.message}`);
      }
      const makePromiser = mod.sqlite3Worker1Promiser;
      const maybePromiser = makePromiser();
      if (typeof maybePromiser === "function") {
        return maybePromiser;
      }
      return await maybePromiser;
    })();
  }
  return promiserPromise;
}
async function ensureOpfs(promiser) {
  const config = await promiser("config-get", {});
  const vfsList = config?.result?.vfsList;
  if (!Array.isArray(vfsList) || !vfsList.includes("opfs")) {
    throw new Error("[op-sqlite] OPFS is required on web for persistence. Ensure COOP/COEP headers are set and OPFS is available in this browser.");
  }
}
async function executeWorker(promiser, dbId, query, params) {
  ...
  const rows = Array.isArray(result?.resultRows) ? result.resultRows : Array.isArray(result) ? result : [];
  const columnNames = Array.isArray(result?.columnNames) ? result.columnNames : rows.length > 0 ? Object.keys(rows[0] ?? {}) : [];
...
function enhanceWebDb(db, options) {
  const lock = {
    queue: [],
    inProgress: false
  };
  ...
  const withTransactionLock = async work => {
    return new Promise((resolve, reject) => {
      const tx = {
        start: () => {
          work().then(resolve).catch(reject).finally(() => {
  ...
  const unsupported = method => () => throwSyncApiError(method);
  const enhancedDb = {
    ...
    transaction: async fn => {
      return withTransactionLock(async () => {
        ...
        const commit = async () => {
          if (finalized) {
            throw new Error(`OP-Sqlite Error: Database: ${options.name}. Cannot execute query on finalized transaction`);
          }
...
async function createWebDb(params) {
  ...
  return {
    ...
    executeWithHostObjects: async (query, bind) => {
      return executeWorker(promiser, dbId, query, bind);
    },
    executeBatch: async _commands => {
...
export const getDylibPath = (_bundle, _name) => {
