...
function enhanceDB(db, options) {
  ...
  const enhancedDb = {
    ...
    executeWithHostObjects: async (query, params) => {
      return params ? await db.executeWithHostObjects(query, params) : await db.executeWithHostObjects(query);
    },
    executeRaw: async (query, params) => {
      return db.executeRaw(query, params);
    },
    executeRawSync: (query, params) => {
      return db.executeRawSync(query, params);
    },
